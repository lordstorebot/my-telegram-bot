...
import httpx
from database import init_db, update_balance, get_balance
...
async def my_balance(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        balance = get_balance(user_id)  # أو user_balances.get(user_id, 0) إذا كنت تستخدم الذاكرة
        ...
        if update.callback_query:
            await update.callback_query.edit_message_text("❌ حدث خطأ في عرض الرصيد")
        else:
...
async def handle_game_id(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    option = user_data.get('selected_option')
    if not option:
        await update.message.reply_text("❌ لم يتم اختيار نوع شحن، يرجى البدء من جديد.")
        return
    ...
    charge_price = user_data.get('charge_price')
    if not charge_price:
        await update.message.reply_text("❌ لم يتم تحديد سعر الشحن، يرجى المحاولة لاحقًا.")
        return
    ...
    try:
        ...
        await update.message.reply_text("❌ حدث خطأ في التحقق من الرصيد، يرجى المحاولة لاحقًا.")
        return
...
async def remove_ads(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.set_default_administrator_rights()
    await update.message.reply_text("تم تعطيل العناصر غير المرغوب بها ✅")

def main() -> None:
    init_db() 
    application = Application.builder().token(TOKEN).build()

    application.add_handler(CommandHandler("remove_ads", remove_ads))
